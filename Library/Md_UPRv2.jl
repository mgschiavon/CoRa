...
#   considering B sequestration by I binding
...
module mm
	...
	odeFB = @ode_def begin
		...
		dI   = mI - (gI * I) - (cB * B * I) + (gIB * IB) - (cA * (U + Ud + UdB) * I) + (gIA * ((kI ^ nI)/((kI ^ nI) + (Ia ^ nI))) * Ia)
		...
		dB   = (bB * (1 + (nB * (Hs ^ 2)/(a0 + (a1 * Hs) + (Hs ^ 2))))) - (gB * B) - (cB * U * B) + (gUB * UB) - (cB * Ud * B) + (gUB * UdB) + (gF * UB) + (gI * IB) - (cB * B * I) + (gIB * IB)
	...
	odeNF = @ode_def begin
		...
		dUd  = - (cB * Ud * B) + (gUB * UdB) + (cD * U) - (cE * E * Ud) + (gB * UdB)
		dUdB = (cB * Ud * B) - (gUB * UdB) + (cD * UB) - (cE * E * UdB) - (gB * UdB)
		dI   = mI - (gI * I) - (cB * B * I) + (gIB * IB) - (cA * uT * I) + (gIA * ((kI ^ nI)/((kI ^ nI) + (Ia ^ nI))) * Ia)
		dIB  = - (gI * IB) + (cB * B * I) - (gIB * IB)
