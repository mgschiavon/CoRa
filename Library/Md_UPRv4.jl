# Unfolded protein response (v04)
#   with explicit B & E mRNA dynamics
#   & alternative interpretation of IA inactivation function
...
module mm
	...
	odeFB = @ode_def begin
		...
		dI   = mI - (gI * I) - (cB * B * I) - (cA * (U + Ud + UdB) * I) + (gIB * IB) + (gIA * ((kI ^ nI)/((kI ^ nI) + (Ia ^ nI))))
		dIB  = - (gI * IB) + (cB * B * I) - (gIB * IB)
		dIa  = - (gI * Ia) + (cA * (U + Ud + UdB) * I) - (gIA * ((kI ^ nI)/((kI ^ nI) + (Ia ^ nI))))
