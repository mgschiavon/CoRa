...
module mm
	...
	odeFB = @ode_def begin
		...
		dB   = (bB * Bm) - (gB * B) - (cB * U * B) + (gUB * UB) - (cB * Ud * B) + (gUB * UdB) + (gF * UB)
		dEm  = (bEm * (1 + (nE * (Hs ^ 2)/(a0 + (a1 * Hs) + (Hs ^ 2))))) - (gEm * Em)
		dE   = (bE * Em) - (gE * E)
		dRm  = - (gHs * Rm) + (bHs * min(Ia,Hu))
		dRs  = (0.00833 * Rm) - (0.0000347 * Rs)
	end sU cB gUB cD cE gB gF cA gIB gIA kI nI bHu gHs bHs bBm nB a0 a1 gBm bB gB bEm nE gEm bE gE uT;
